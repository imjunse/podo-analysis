...
review_method: "ooo pm interview + prototype review 2026-03-27"
projectInfo:
  name: "PODO"
  ...
  enums:
    ...
    onboarding_step: [INTRO, LANGUAGE, GOAL, EXPERIENCE, FREQUENCY, PREFERRED_TIME, CLASS_STYLE, CLASS_LENGTH, RESULT, BOOKING, COMPLETE, PUSH, MARKETING, FINAL]
    language: [en, ja]
    experience_level: [1, 2, 3, 4, 5]
    class_length_preference: [10, 25, 40]
    goals: [BUSINESS, TRAVEL, EXAM, GROWTH, MEDIA, FRIENDS]
    class_style: [CURRICULUM, FREE_TALK, CORRECTION, RELAXED]
inputDocuments:
  ...
  surface_prototype: { path: "surface/prototype/index.html" }
  v1_reference:
...
changeLog:
  ...
  - { event: "prototype.reviewed", date: "2026-03-27", summary: "실사 프로토타입 리뷰 반영 — 인트로 UX, 로그인 디자인, 옵션 카드, 학습 목표 6개, 수업 스타일 4개/복수선택, 예약 후 4단계 플로우, 마케팅 쿠폰 인센티브 반영. PRD v2.0 전면 업데이트." }
---
...
| ClickHouse 이벤트 DB | 체험 구매 유저 89.5%가 5분 이내 결정 (v1 참고), 체험 완료율 약 50% |
...
- 예약 후 플로우를 4단계로 분리 (예약 완료 → 알림 받기 → 마케팅 동의 → 최종 완료)
- 마케팅 동의 인센티브로 "25분 무료 체험 쿠폰" 제공 (동의 시)
...
| 체험 완료율 | 약 50% | +10% 향상 (독려 카드 + 쿠폰 인센티브) |
| personalization 데이터 수집률 | 0% (수집 안 함) | 온보딩 완료 유저의 95%+ |
| 마케팅 동의율 | 낮음 (인센티브 없음) | 마케팅 동의 화면 노출 대비 측정 |
...
|---------|-------------|-------------|
| 인트로 화면 | 진행률 바 + 우측 상단 건너뛰기 + 단일 "다음" CTA | 도트 인디케이터 → 프로그레스 바, 건너뛰기 위치 변경 |
| 인트로 슬라이드 카피 | "수업하면 편하게" (2번 슬라이드) | "전화하면" → "수업하면" 수정 |
| 로그인 화면 | 쿠폰 헤드라인 + PODO 캐릭터 + 말풍선 + "카카오로 3초만에 로그인" + "로그인이 안되시나요?" | 실사 앱 디자인 반영 |
| 사전 안내 화면 | "7가지만 물어볼게요!" 질문 수 사전 고지 | NEW |
| 언어 선택 | 영어/일본어 중 1개 선택 | v1 유지 |
| 학습 목표 | 복수 선택 6개 — 업무/여행/시험/자기계발/드라마·미디어/외국인 친구 | 4개 → 6개 확대, 취미 분리 |
| 말하기 경험 | "지금 실력이 어느 정도예요?" — 5단계 짧은 단답형 | 질문 문구·옵션 워딩 개선 |
| 수업 빈도 | 주 몇 회 수업할지 선택 | NEW |
| 선호 시간대 | 수업 선호 시간대 선택 | NEW |
| 수업 스타일 | 4개 복수 선택 — 교재 기반/프리토킹/교정 받기/편한 수업 | 옵션 전면 개편, 복수 선택 |
| 수업 길이 | 10분/25분/40분 선호도 (리서치 목적) | NEW |
| 옵션 카드 UI | 선택 시 배경색만 활성화 (체크박스/라디오 없음) | 전체 질문 화면에 일괄 적용 |
| 나의 학습 플랜 결과 | 개인화 추천 메시지 | 추가 쿠폰 안내 제거 |
| 체험 예약 | 레벨 유저 직접 선택(워딩 구체화) + 선호 시간대 프리셋 | 레벨 테스트 → 자가 선택 |
| 예약 완료 | 예약 정보 요약 + 체험 완료 독려 카드 (긴장 해소 + 예습 교재) + "확인" CTA | 간소화 + 독려 카드 추가 |
| 알림 받기 | 전용 페이지 + iOS 시스템 팝업 | 별도 화면으로 분리 |
| 마케팅 동의 | "25분 무료 체험 쿠폰" 인센티브 + 약관 팝업 | NEW (인센티브 도입) |
| 최종 완료 | "모든 준비 완료!" + "수업 전 예습하기" + "메인으로 가기" | NEW (별도 화면) |
| 레벨 테스트 | 삭제 | REMOVED |
...
| 마케팅 쿠폰 실제 발급 로직 | 다음 PRD (체험 신청~완료 플로우) |
| 인앱 알림 | 나중에 별도 추가 예정 |
...
1. **인트로 화면** — 앱 실행 시 PODO 캐릭터가 등장. 3화면 진행률 바 기반 슬라이드:
   - "외국인 앞에 서면 머릿속이 하얘지나요?"
   - "수업하면 편하게 말할 수 있어요"
   - "25분 무료 체험, 한 번이면 충분해요"

   우측 상단 "건너뛰기"로 skip 가능. "다음" 버튼 하나로 진행. 수진은 "교재로 미리 준비할 수 있구나" 하고 안심한다. "시작하기" 버튼을 누른다.

2. **로그인** — 쿠폰 헤드라인 + PODO 캐릭터 + 말풍선("무료 체험 시작해볼까요?") + "카카오로 3초만에 로그인" CTA. 기존 OAuth 플로우 그대로.
...
5. **학습 목표** (`/onboarding/goal`) — "영어 배우는 이유가 뭐예요?" — "자기계발·성장" 선택.

6. **말하기 경험** (`/onboarding/experience`) — "지금 실력이 어느 정도예요?" PODO 캐릭터 말풍선. "긴장돼서 아는 것도 못 말해요" 선택. → 내부적으로 experience_level=2, 기본 레벨 "초급" 세팅.

7. **수업 빈도** (`/onboarding/frequency`) — "일주일에 몇 번 수업하고 싶으세요?" 주 2회 선택.

8. **선호 시간대** (`/onboarding/preferred-time`) — "언제 수업하면 좋을까요?" 평일 저녁 선택.

9. **수업 스타일** (`/onboarding/class-style`) — "어떤 수업이 좋으세요?" 교재 기반 + 프리토킹 선택 (복수 가능).
...
13. **예약 완료** (`/onboarding/complete`) —
    - 예약 정보 요약 (날짜·시간·레벨)
    - 독려 카드: "긴장하지 마세요! 수업 전 예습 교재로 미리 준비할 수 있어요"
    - "확인" CTA → 알림 받기 화면으로 이동

14. **알림 받기** (`/onboarding/push`) —
    - "수업 알림을 받아보세요!" 안내 화면
    - "알림 받기" 버튼 → iOS 시스템 팝업 표시
    - 허용/거부 후 마케팅 동의 화면으로 이동

15. **마케팅 동의** (`/onboarding/marketing`) —
    - "지금 동의하면 25분 무료 체험 쿠폰을 드려요!" 인센티브 배너
    - "동의하고 쿠폰 받기" / "다음에" 선택
    - 동의 시 약관 팝업 표시 → 확인 후 최종 완료 화면

16. **최종 완료** (`/onboarding/final`) —
    - "모든 준비 완료!"
    - "수업 전 예습하기" CTA (예습 교재 탭으로 이동)
    - "메인으로 가기" CTA
...
**Rising Action:**
인트로(skip) → 카카오 로그인 → 사전 안내 "7가지만 물어볼게요!" → 일본어 선택 → "업무·비즈니스" → "일상 대화는 괜찮은데, 전문 주제는 어려워요" (experience_level=4) → 주 3회 → 주말 오전 → "특정 주제 위주 + 교정 받기" (복수 선택) → 25분

**Climax:**
학습 플랜 결과: "비즈니스 일본어 실력을 한 단계 올려볼까요! 실력에 맞는 수업 준비했어요." 체험 예약: "중급" 레벨 (4단계 전부 노출, 변경 가능).

**Resolution:**
예약 완료 (독려 카드) → 알림 받기 → 마케팅 동의 (쿠폰 수락) → 최종 완료. 민수는 "비즈니스 주제로 연습할 수 있겠네" 하고 기대한다.
...
**Rising Action:**
앱 재실행 → 로그인 유지 상태 → 시스템이 `onboardingStatus=IN_PROGRESS`, `currentStep=FREQUENCY` 감지 → 수업 빈도 질문 화면부터 자동 재개. 이전에 입력한 언어(영어), 학습 목표(자기계발), 말하기 경험(긴장)은 이미 저장되어 있어 다시 묻지 않음.
...
**Resolution:**
예약 완료 → 알림 받기 → 마케팅 동의 → 최종 완료. 지은은 "이전에 했던 거 다시 안 해도 되고, 뒤로가기도 되네" 하며 완료.
...
**Resolution:**
체험 예약 완료 → 알림 받기 → 마케팅 동의 → 최종 완료. 또는 AI 학습탭 진입. 시스템은 10분 선호 데이터를 수집하여 향후 10분 수업 기획에 활용.
...
### DR-003: 수업 스타일 데이터 (source: CST-005, PM Review, Prototype Review)
- 수업 스타일 선택지: **교재 기반** / **프리토킹** / **교정 받기** / **편한 수업** (4개, 복수 선택 가능).
- 학습 목표(Goal)와 내용 중복 없도록 학습 방식 중심으로 구성.
- **첫 출시(v2.0)**: 데이터 수집만. 결과 화면 분기 변수는 말하기 경험(5단계) 1개만 사용.
- **향후 업데이트(v2.x)**: 수업 스타일 데이터를 튜터 매칭 또는 커리큘럼 추천에 활용.

### DR-004: 마케팅 동의 인센티브 쿠폰 (source: Prototype Review)
- 마케팅 동의 화면에서 동의 시 **25분 무료 체험 쿠폰 1장** 인센티브 제공.
- 이 scope에서는 **인센티브 UI 및 동의 처리만 포함**. 실제 쿠폰 발급 로직은 다음 PRD에서 구현.
- "체험 완료 후 다른 언어 무료 체험" 안내 문구는 이 scope에서 제외 (별도 검토).
- 인센티브 배너 위치: 마케팅 동의 화면 (`/onboarding/marketing`).

### DR-005: 알림 동의 타이밍 및 전용 화면 (source: v1 CST-005, Prototype Review)
- iOS 네이티브 알림 팝업은 앱당 1회만 표시 가능.
- 예약 완료 직후 **전용 화면** (`/onboarding/push`)에서 요청. 이 시점이 유저가 알림의 가치를 가장 잘 이해하는 시점.
- 전용 화면에서 "알림 받기" 버튼 탭 → iOS 시스템 팝업 표시 → 허용/거부 결과 저장.
...
Response 200:
{
  ...
  "classStyle": ["CURRICULUM", "FREE_TALK"] | null,
...
| `GOAL` | `goals: string[]` | 학습 목표 배열 (BUSINESS/TRAVEL/EXAM/GROWTH/MEDIA/FRIENDS) |
| `EXPERIENCE` | `experienceLevel: 1~5` | 말하기 경험 |
| `FREQUENCY` | `frequency: number` | 주 N회 |
| `PREFERRED_TIME` | `preferredTime: string` | 선호 시간대 |
| `CLASS_STYLE` | `classStyle: string[]` | 수업 스타일 배열 (복수 선택, CURRICULUM/FREE_TALK/CORRECTION/RELAXED) |
| `CLASS_LENGTH` | `classLengthPreference: 10 \| 25 \| 40` | 수업 길이 선호 |
| `BOOKING` | `bookingId: number` | 체험 예약 ID |
| `PUSH` | `agreed: boolean` | 알림 동의 |
| `MARKETING` | `marketingAgreed: boolean` | 마케팅 동의 |
| `FINAL` | (없음) | 온보딩 최종 완료 |
...
CREATE TABLE GT_USER_ONBOARDING (
    ...
    goals                 VARCHAR(200) NULL
                          COMMENT '복수 선택, 쉼표 구분: BUSINESS,TRAVEL,EXAM,GROWTH,MEDIA,FRIENDS',
    ...
    class_style           VARCHAR(100) NULL
                          COMMENT '복수 선택, 쉼표 구분: CURRICULUM,FREE_TALK,CORRECTION,RELAXED',
...
- `class_style`: 단일 값 → 복수 선택 저장 (쉼표 구분 문자열, 최대 100자)
- `goals`: 4개 → 6개 enum 값 지원 (BUSINESS/TRAVEL/EXAM/GROWTH/MEDIA/FRIENDS)
...
├── intro/page.tsx              ← 인트로 (진행률 바 + 단일 "다음" CTA + 우측 상단 건너뛰기)
├── language/page.tsx           ← 언어 선택
├── goal/page.tsx               ← 학습 목표 (6개, 복수 선택)
├── experience/page.tsx         ← 말하기 경험 (5개 단답형)
├── frequency/page.tsx          ← 수업 빈도
├── preferred-time/page.tsx     ← 선호 시간대
├── class-style/page.tsx        ← 수업 스타일 (4개, 복수 선택)
├── class-length/page.tsx       ← 수업 길이
├── result/page.tsx             ← 학습 플랜 결과
├── booking/page.tsx            ← 체험 예약
├── complete/page.tsx           ← 예약 완료 (독려 카드)
├── push/page.tsx               ← 알림 받기 (전용 페이지 + 시스템 팝업)
├── marketing/page.tsx          ← 마케팅 동의 (쿠폰 인센티브)
└── final/page.tsx              ← 최종 완료
...
└── ui/
    ...
    ├── OptionCard.tsx           ← 선택지 카드 (배경색 활성화만, 체크박스/라디오 없음)
    └── IntroSlider.tsx          ← 인트로 슬라이드 (진행률 바)
```
...
- **FR-001:** 앱 첫 실행 시 PODO 캐릭터가 등장하는 3화면 슬라이드 인트로 표시 (source: brief.md)
- **FR-002:** 상단 진행률 바(progress bar)로 현재 슬라이드 위치 표시. 도트 인디케이터 미사용. (source: Prototype Review)
- **FR-003:** 우측 상단 "건너뛰기" 버튼으로 skip 가능. 각 슬라이드 하단의 "다음" 버튼 1개만 CTA로 노출. (source: Prototype Review)
- **FR-004:** 마지막 슬라이드에서 "시작하기" 버튼 → 로그인 화면으로 이동 (source: v1)
- **FR-005:** 인트로는 로그인 전이므로 서버 저장 불필요. 로컬 상태만 관리 (source: v1 build-spec)
- **FR-006:** 인트로 2번 슬라이드 문구: "수업하면 편하게 말할 수 있어요" (source: Prototype Review)

### 로그인 화면 (`/callback/oauth/`)

- **FR-007:** 화면 구성 (위에서 아래): 무료 체험 쿠폰 헤드라인 → PODO 캐릭터 → 캐릭터 말풍선("무료 체험 시작해볼까요?") → "카카오로 3초만에 로그인" CTA 버튼 → "로그인이 안되시나요?" 링크 (source: Prototype Review, 실사 앱 디자인 반영)
- **FR-008:** 기존 OAuth 플로우 그대로 유지. UI만 변경. [BROWNFIELD]

### 사전 안내 화면

- **FR-009:** 로그인 직후, PODO 캐릭터 말풍선: "반가워요! 딱 맞는 수업을 추천해 드릴게요. 7가지만 물어볼게요!" (source: Exploration Phase 4, PM Review)
- **FR-010:** 질문 수를 사전 고지하여 유저의 심리적 부담 감소 (source: 모지(MojiMoji) 레퍼런스)

### 언어 선택 (`/onboarding/language`)

- **FR-011:** "어떤 언어를 배우고 싶으세요?" — PODO 말풍선으로 질문 (source: v1)
- **FR-012:** 영어/일본어 중 1개만 선택 가능 (source: v1)
- **FR-013:** 선택 → PATCH /onboarding/step → 다음 화면 이동 (source: v1 build-spec)

### 학습 목표 (`/onboarding/goal`)

- **FR-014:** "영어/일본어 배우는 이유가 뭐예요?" — PODO 말풍선 (source: v1)
- **FR-015:** 복수 선택 가능 — 6개 문장형 옵션 (source: Prototype Review):
  - 업무·비즈니스에 쓰고 싶어요 (BUSINESS)
  - 여행할 때 쓰고 싶어요 (TRAVEL)
  - 시험이나 자격증이 필요해요 (EXAM)
  - 자기계발·성장을 하고 싶어요 (GROWTH)
  - 드라마·영화·미디어를 즐기고 싶어요 (MEDIA)
  - 외국인 친구를 사귀고 싶어요 (FRIENDS)
- **FR-016:** 1개 이상 선택 시 다음 버튼 활성화 (source: v1)
- **FR-017:** 옵션 카드 선택 시 체크박스/라디오 없이 배경색만 변경으로 활성화 표시. 전체 옵션 카드에 동일하게 적용. (source: Prototype Review)

### 말하기 경험 (`/onboarding/experience`)

- **FR-018:** "지금 실력이 어느 정도예요?" — PODO 말풍선 (source: Prototype Review, 기존 "말할 때 어떤 느낌이에요"에서 변경)
- **FR-019:** 5단계 단일 선택, 짧은 단답형 옵션 (source: Prototype Review):
  - ① 아직 기초부터 배워야 해요
  - ② 긴장돼서 아는 것도 못 말해요
  - ③ 간단한 건 되는데 길어지면 막혀요
  - ④ 일상 대화는 되는데 전문 주제는 어려워요
  - ⑤ 대부분 상황에서 자유롭게 말해요
- **FR-020:** 선택 기반으로 기본 레벨 자동 세팅 (source: DR-001)

### 수업 빈도 (`/onboarding/frequency`)

- **FR-021:** "일주일에 몇 번 수업하고 싶으세요?" — PODO 말풍선 (source: Exploration Phase 4)
- **FR-022:** 주 1~7회 중 선택 (source: align-packet)

### 선호 시간대 (`/onboarding/preferred-time`)

- **FR-023:** "언제 수업하면 좋을까요?" — PODO 말풍선 (source: Exploration Phase 4)
- **FR-024:** 평일 아침/점심/저녁, 주말 아침/점심/저녁 중 선택 (source: align-packet)
- **FR-025:** 선택한 시간대는 체험 예약 화면에서 프리셋으로 적용 (source: align-packet)

### 수업 스타일 (`/onboarding/class-style`)

- **FR-026:** "어떤 수업이 좋으세요?" — PODO 말풍선 (source: Exploration Phase 5)
- **FR-027:** 4개 복수 선택 (source: Prototype Review, 학습 목표와 중복 제거, 학습 방식 중심으로 구성):
  - 교재 보고 체계적으로 배우고 싶어요 (CURRICULUM)
  - 주제 없이 자유롭게 이야기하고 싶어요 (FREE_TALK)
  - 틀린 표현을 바로바로 고쳐주면 좋겠어요 (CORRECTION)
  - 부담 없이 편하게 연습하고 싶어요 (RELAXED)
- **FR-028:** 1개 이상 선택 시 다음 버튼 활성화 (source: FR-017 패턴 동일)
- **FR-029:** 수집 데이터는 v2.0에서 DB 저장만. 결과 화면 분기 미사용. (source: DR-003)

### 수업 길이 (`/onboarding/class-length`)

- **FR-030:** "한 번에 몇 분 수업이 좋으세요?" — PODO 말풍선 (source: Exploration Phase 4 Round 2)
- **FR-031:** 10분/25분/40분 중 선택 (source: align-packet)
- **FR-032:** 리서치 목적 수집. 실제 수업은 25분 고정 (source: DR-002)

### 나의 학습 플랜 결과 (`/onboarding/result`)

- **FR-033:** PODO 캐릭터가 입력 기반 개인화 학습 플랜 표시 (source: align-packet)
- **FR-034:** 결과 구성 요소:
...
- **FR-035:** 10분 선택 유저: AI 수업 CTA 추가 표시 (source: Exploration Phase 4 Round 2)
- **FR-036:** 40분 선택 유저: "25분이 집중력에 딱 좋아요" 안내 (source: Exploration Phase 5)
- **FR-037:** 추가 쿠폰 안내 ("체험 완료하면 다른 언어도 무료 체험") 미노출. 쿠폰 인센티브는 마케팅 동의 화면에서만 제공. (source: Prototype Review, DR-004)

### 체험 예약 (`/onboarding/booking`)

- **FR-038:** 말하기 경험 기반 기본 레벨 프리셋 (source: DR-001) [BROWNFIELD: 기존 예약 API 활용]
- **FR-039:** 레벨 설명은 구체적 설명형 워딩 사용 (source: CST-002)
- **FR-040:** 유저가 레벨을 직접 변경 가능 + "언제든 변경할 수 있어요" 안내 (source: align-packet)
- **FR-041:** 선호 시간대 프리셋 적용 — 온보딩에서 선택한 시간대 기반 (source: align-packet)
- **FR-042:** 기존 체험 예약 API 사용 [BROWNFIELD] (source: v1 build-spec)

### 예약 완료 (`/onboarding/complete`)

- **FR-043:** 예약 정보 요약 표시 — 날짜·시간·레벨 (source: Prototype Review)
- **FR-044:** 체험 완료 독려 카드 표시:
  - "긴장하지 마세요!" 안심 메시지
  - "수업 전 예습 교재로 미리 준비할 수 있어요" 예습 유도
  (source: Prototype Review, 체험 완료율 50% 개선 목적)
- **FR-045:** "확인" 버튼 CTA → 알림 받기 화면(`/onboarding/push`)으로 이동 (source: Prototype Review)
- **FR-046:** 마케팅 동의 체크박스 미노출. 별도 화면(`/onboarding/marketing`)에서 처리. (source: Prototype Review)

### 알림 받기 (`/onboarding/push`)

- **FR-047:** 전용 화면에서 수업 알림 동의 유도 (source: Prototype Review, DR-005)
- **FR-048:** "알림 받기" 버튼 탭 → iOS/Android 시스템 알림 권한 팝업 표시 (source: DR-005)
- **FR-049:** 허용/거부 결과를 PATCH /onboarding/step (step: PUSH)로 저장 (source: DR-005)
- **FR-050:** 알림 팝업 결과에 관계없이 다음 화면(마케팅 동의)으로 진행 (source: DR-005)

### 마케팅 동의 (`/onboarding/marketing`)

- **FR-051:** 인센티브 배너 표시: "지금 동의하면 25분 무료 체험 쿠폰을 드려요!" (source: Prototype Review, DR-004)
- **FR-052:** "동의하고 쿠폰 받기" CTA → 약관 팝업(bottom sheet) 표시 → 확인 시 동의 처리 (source: Prototype Review)
- **FR-053:** "다음에" 링크 → 비동의 처리 후 최종 완료 화면으로 이동 (source: Prototype Review)
- **FR-054:** 동의 여부를 PATCH /onboarding/step (step: MARKETING, marketingAgreed: boolean)으로 저장 (source: DR-004)

### 최종 완료 (`/onboarding/final`)

- **FR-055:** "모든 준비 완료!" 완료 메시지 표시 (source: Prototype Review)
- **FR-056:** "수업 전 예습하기" CTA → 예습 교재 탭으로 이동 (source: Prototype Review)
- **FR-057:** "메인으로 가기" CTA → 홈 화면 이동 (source: v1)
- **FR-058:** 최종 완료 시 POST /onboarding/complete 호출하여 onboardingStatus=COMPLETED 처리 (source: API-003)

### 온보딩 상태 추적

- **FR-059:** 신규 유저 생성 시 GT_USER_ONBOARDING 레코드 자동 생성 (status=NOT_STARTED) (source: v1 build-spec)
- **FR-060:** 각 단계 완료 시 currentStep 업데이트 — 이탈 복구 지점 기록 (source: CST-003)
- **FR-061:** 이전 step 미완료 상태에서 다음 step 저장 시도 → 400 에러 (source: v1 build-spec)
- **FR-062:** 재방문 시 currentStep 기반 자동 재개 (source: align-packet 시나리오 3)
- **FR-063:** 뒤로가기로 이전 화면 이동 → 이전 응답 수정 가능 (단순 폼 다시 채우기, 대화 맥락 유지 없음) (source: PM Review)

### 라우팅 분기

- **FR-064:** 피처 플래그 ON 시 HomeRedirection 로직 변경 (source: v1 build-spec):
...
- **FR-065:** 기존 /subscribes/trial 접근 시 피처 플래그 ON이면 새 온보딩으로 리다이렉트 [BROWNFIELD] (source: v1)

### UX 전체

- **FR-066:** 전체 온보딩 UX — PODO 캐릭터 말풍선 비주얼의 스텝 폼. **빠른 진행 우선, 애니메이션 최소화**. 채팅형 히스토리 누적 UX 아님 — 각 질문은 독립된 화면. (source: brief.md, PM Review)
- **FR-067:** 한 화면 = 한 질문 원칙 (source: Exploration Phase 4)
- **FR-068:** 진행률 바 표시 — PODO 연두색(#B5FD4C) (source: Surface Preview)
...
|------|----------|-------------------|
| QA-001 | 신규 유저 전체 플로우 완주 | 인트로 → 로그인 → 7개 질문 → 결과 → 예약 → 완료 → 알림 → 마케팅 → 최종. 모든 데이터 GT_USER_ONBOARDING에 저장 |
...
### 수업 스타일 복수 선택 (Priority: MEDIUM)

| Case | Scenario | Expected Handling |
|------|----------|-------------------|
| QA-012 | 수업 스타일 1개 선택 | classStyle=["CURRICULUM"] 저장 |
| QA-013 | 수업 스타일 복수 선택 | classStyle=["CURRICULUM","FREE_TALK"] 저장 |
| QA-014 | 수업 스타일 미선택 | 다음 버튼 비활성화 |

### 예약 후 플로우 (Priority: HIGH)

| Case | Scenario | Expected Handling |
|------|----------|-------------------|
| QA-015 | 예약 완료 → "확인" | 알림 받기 화면으로 이동 |
| QA-016 | 알림 허용 | notification_agreed=true 저장 → 마케팅 동의 화면 |
| QA-017 | 알림 거부 | notification_agreed=false 저장 → 마케팅 동의 화면 |
| QA-018 | 마케팅 동의 | marketing_agreed=true 저장, 쿠폰 안내 표시 → 최종 완료 |
| QA-019 | 마케팅 미동의 | marketing_agreed=false 저장 → 최종 완료 |

### 피처 플래그 (Priority: HIGH)

| Case | Scenario | Expected Handling |
|------|----------|-------------------|
| QA-020 | 피처 플래그 OFF | 기존 /subscribes/trial 플로우 유지 |
| QA-021 | 피처 플래그 ON + 기존 유저 | onboardingStatus 없는 기존 유저 → COMPLETED 간주 → 홈 |
| QA-022 | 피처 플래그 ON + 신규 유저 | 새 온보딩 플로우 진입 |
...
| `onboarding_push_viewed` | — | 알림 받기 화면 진입 |
| `onboarding_notification_agreed` | `agreed: boolean` | 알림 동의 결과 |
| `onboarding_marketing_viewed` | — | 마케팅 동의 화면 진입 |
...
| 4 | 마케팅 동의 쿠폰 최종 워딩 | PM | 미확정 | ❌ 개발 병행 가능 | 플레이스홀더로 개발 진행 |
| 5 | 수업 스타일 선택지 최종 목록 | PM + 커리큘럼팀 | 프로토타입 기반 초안 확정, 최종 리뷰 필요 | ❌ 첫 출시 분기에 미사용 | 커리큘럼팀 리뷰 후 확정 |
...
### 착수 가능 조건 (현재 충족)
- [x] PRD 확정
- [x] sprint-kit HTML 프로토타입 존재 (prototype review 반영 완료)
...
| `research_class_style_selected` | `styles: string[]` | 수업 스타일 질문 응답 |
...
| 인트로 인디케이터 | 도트 | 진행률 바 |
| 인트로 건너뛰기 위치 | 미정 | 우측 상단 고정 |
| 로그인 화면 | 기본 | 쿠폰 헤드라인 + 캐릭터 + 말풍선 + 카카오 CTA |
| 옵션 카드 UI | 체크박스/라디오 | 배경색 활성화만 |
| 학습 목표 선택지 | 4개 | 6개 (취미 → 자기계발·미디어·외국인 친구로 분화) |
| 수업 스타일 | 미정 | 4개 복수 선택 (교재/프리토킹/교정/편한 수업) |
| 결과 화면 | 테스트 점수 + 추천 레벨 + 안심 메시지 | 개인화 학습 플랜 + 분기별 메시지 |
| 예약 후 플로우 | 단일 완료 화면 | 4단계 분리 (예약 완료 → 알림 → 마케팅 → 최종) |
| 마케팅 동의 위치 | 결과 화면 체크박스 | 전용 화면 (쿠폰 인센티브) |
| 추가 쿠폰 | 없음 | 마케팅 동의 시 "25분 무료 체험 쿠폰" |
| 체험 완료 독려 | 없음 | 예약 완료 화면 독려 카드 (긴장 해소 + 예습) |
| DB 스키마 | quiz_score, experience_score, total_score | frequency, preferred_time, class_style (복수), class_length_pref |
...
| OPEN-003 | 마케팅 동의 쿠폰 안내 문구 최종 워딩 | PO | 확정 필요 |
| OPEN-004 | 수업 스타일 선택지 최종 목록 | PO + 커리큘럼팀 | 프로토타입 기반 초안, 최종 리뷰 필요 |

### 텍스트 와이어프레임 참조

Surface Prototype 파일: `surface/prototype/index.html`
- PODO 캐릭터(연두색 #B5FD4C 원형) + 말풍선 인터랙션
- 폰 프레임(375x812) 기반 모바일 프로토타입
- 진행률 바, 선택지 카드 (배경색 활성화), CTA 버튼 등 UI 컴포넌트 포함
- 예약 후 4단계 플로우 (complete → push → marketing → final) 포함

---

## 추후 검토 필요 (PRD 반영 보류)

- **온보딩 질문 7개 → 5개 축소 검토**: 수업 빈도(4번)와 선호 시간(5번)을 온보딩에서 제거하고, 빈도는 유료 전환 시 요금제 선택 플로우에서, 선호 시간은 예약 시간 슬롯 선택으로 대체 가능. "7가지만 물어볼게요" → "5가지만 물어볼게요"로 체감 부담 감소 기대
- **시작 안내 화면(0.6) 간소화 검토**: 인트로에서 이미 설명하므로 중복 가능성
