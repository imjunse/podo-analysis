...
회원가입           7,452명  ─── 100%
    │
    ▼ 66.0%
무료체험 결제(신청)  4,916명  ─── 34.0% 이탈  ★ 스냅샷에서 과소평가된 구간
    │
    ▼ 51.6%
체험 완료          2,536명  ─── 48.4% 이탈  ★ 최대 이탈 구간
    │
    ▼  (마케터 분석 연결)
...
### 최대 이탈: 체험결제 → 체험완료 (48.4% 이탈)

무료체험을 결제(신청)한 4,916명 중 **2,380명이 체험 수업을 완료하지 않음**. 이 구간은 예약 → 수업 참석(노쇼) 문제를 포함하며, 온보딩 퍼널 개선의 최우선 과제.

### 새로 드러난 이탈: 가입 → 체험결제 (34.0% 이탈)

가입자 7,452명 중 **2,536명(34.0%)이 무료체험을 신청하지 않음**. 스냅샷 기반에서는 77.5%로 양호해 보여 간과되었으나, 유저 기반으로 보면 실제 전환율은 66.0%로 가입자의 3분의 1이 체험조차 신청하지 않는다. 코호트 분석으로 새롭게 드러난 개선 포인트.
