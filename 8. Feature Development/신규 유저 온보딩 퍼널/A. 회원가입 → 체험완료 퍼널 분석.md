...
회원가입           7,452명  ─── 100%
    │
    ▼ 77.5%
무료체험 결제(신청)  5,773명  ─── 22.5% 이탈
    │
    ▼ 50.7%
체험 완료          2,927명  ─── 49.3% 이탈  ★ 최대 이탈 구간
    │
    ▼  (마케터 분석 연결)
정규 결제          ~1,032명  ─── ~13.8% (가입 대비)
```
...
| 퍼널 단계                   | 전환율       |
| ------------------------- | ---------: |
| 회원가입 → 무료체험 결제(신청)   | **77.5%**  |
| 무료체험 결제(신청) → 체험 완료   | **50.7%**  |
| **회원가입 → 체험 완료 (전체)** | **39.3%**  |
...
### 가장 큰 이탈: 체험결제 → 체험완료 (49.3% 이탈)

무료체험을 결제(신청)한 5,773명 중 **절반에 가까운 2,846명이 체험 수업을 완료하지 않음**. 이 구간은 예약 → 수업 참석(노쇼) 문제를 포함하며, 온보딩 퍼널 개선의 최우선 과제.
...
### 가입 → 체험결제 전환율은 양호 (77.5%)

가입자의 약 4분의 3이 무료체험을 신청. 현재 `trial_free` 플래그로 무료 체험이 활성화되어 있어 결제 장벽이 낮음.

### 뒷단 분석과의 숫자 일치 확인

- 우리 체험 완료: **2,927명** (유니크 유저)
- 마케터(박서연) 체험 완료 주차 합산: **2,906건** (차이 0.7%)
...
가입 7,452명 중 약 1,032명만 정규 결제까지 도달 (마케터 분석 기준 체험완료→정규결제 전환율 ~35% 적용)
...
회원가입 → 체험결제 → 체험완료     ───→    체험완료 → 정규레슨 결제
  7,452    5,773     2,927               2,927    ~1,032건
  (77.5%)  (50.7%)                       (31~39% - 하락 추세)
```
